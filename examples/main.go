...
func main() {
	...
	go func() {
		defer wg.Done()
		ticker := time.NewTicker(2 * time.Second) // 改为每2秒发布一次，避免过高频率阻塞
		defer ticker.Stop()
		...
		for {
			...
			case <-ticker.C:
				...
				slog.Warnf("  Last Message Time: %s", metrics["last_message_time"])
				slog.Warnf("  Last Error Time: %s", metrics["last_error_time"])
			...
			case <-ctx.Done():
				slog.Info("Publisher goroutine shutting down...")
				return
			}
		}
	}()
	...
	if metricsServer != nil {
		...
		if err := metricsServer.Shutdown(shutdownCtx); err != nil {
			slog.Errorf("Failed to shutdown metrics server: %v", err)
		} else {
	...
	// 12. 清理资源
	slog.Info("Starting resource cleanup...")

	// 停止所有消息路由
	slog.Info("Stopping message routes...")
	// defer语句会按相反顺序执行，确保正确清理

	// 移除临时会话
	if err := m.RemoveSession("temp-device"); err != nil {
		slog.Errorf("Failed to remove temp session: %v", err)
	} else {
		slog.Info("Temp session removed successfully")
	}

	// 断开所有连接
	slog.Info("Disconnecting all sessions...")
	m.DisconnectAll()
	...
	slog.Info("Resource cleanup completed")
}
